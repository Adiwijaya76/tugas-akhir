<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Form</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">Kode Form*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtid" placeholder="Masukkan Kode Form" autocomplete="off" maxlength="5">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama Form*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Form" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Menu</label>
						<select class="form-control select2" id="cbomenu">
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Sistem</label>
						<select class="form-control select2" id="cbosistem">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($dtsistem)){
									if(count($dtsistem)>0){
										foreach($dtsistem as $x){
											echo "<option value='".$x->id."'>".$x->nama."</option>";
										}
									}
								}
					...
					<div class="form-group col-md-8 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Icon</label>
						<select class="form-control select2" id="cboicon">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($iccon)){
									if(count($iccon)>0){
...
<script>
	...
	function lihaticon(){
		var icon = $("#cboicon").val();
		$("#blokicon").html('<i class="icon '+ icon +'" aria-hidden="true"></i>')
	}
	...
	function tambah(){
		...
		var menu = $("#cbomenu").val();
		var sistem = $("#cbosistem").val();
		var icon = $("#cboicon").val();
    
        if(id == "" || nama == "" || menu == "" || sistem == "" || icon == ""){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, nama: nama, menu: menu, sistem: sistem, icon: icon},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Form Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Form Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					$("#txtid").val(xx[1]);
					$("#txtnama").val(xx[2]);
					$("#cbomenu").val(xx[3]).change();
					$("#cbosistem").val(xx[4]).change();
					$("#cboicon").val(xx[5]).change();
