<div class="page-header">
	<h1 class="page-title">Rencana Pola Tanam</h1>
		...
		<div class="panel">
			...
			<div class="panel-body">
				<div class="form-row">
                    ...
                    <div class="form-group col-md-12">
						<label class="form-control-label">Varietas Tanaman</label>
						<select class="form-control select2" id="txtvarietas">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($varietas)){
									if(count($varietas)>0){
										foreach($varietas as $x){
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtmasa_tanam" placeholder="Masukkan Masa Tanam" autocomplete="off">
					</div>
...
<script>
	...
	function tambah(){
        ...
        var id = $("#txtid").val();
        var id_varietas = $("#txtvarietas").val();
        var masa_tanam = $("#txtmasa_tanam").val();
    
    
        if(id == "" || id_varietas == "" || masa_tanam == ""){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, id_varietas: id_varietas, masa_tanam: masa_tanam},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Rencana Pola Tanam Kosong !', icon: 'error'});
            return;
		}
		swal("Memproses Data.....", {button: false, closeOnClickOutside: false, closeOnEsc: false});
		swal({
	...
	function filter(el){
		...
		$.ajax({
            ...
            success: function(x){
				...
				if(xx[0] == 1){
					$("#txtid").val(xx[1]);
					$("#txtvarietas").val(xx[2]).change();
					$("#txtmasa_tanam").val(xx[3]);
